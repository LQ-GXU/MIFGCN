...
class STCell(nn.Module):
    ...
    def forward(self, x, edge_index, dtw_edge_index,adj):
        ...
        if self.choice[1] == 1:
            ...
            sp_learned_matrix =F.softmax(F.relu(torch.mm(self.sp_source_embed, self.sp_target_embed)+torch.eye(self.node_num).to('cuda')), dim=1)  
            ...
            if self.choice[2] == 1:
                ...
                dtw_learned_matrix = F.softmax(F.relu(torch.mm(self.dtw_source_embed, self.dtw_target_embed)+torch.eye(self.node_num).to('cuda')), dim=1) 
            ...
            output = self.output_linear(cell_output).reshape(-1,self.node_num,self.seq_len*self.num_of_x).unsqueeze(1)
...
class D_GCN(nn.Module):
    ...
    def forward(self, x, support: list, power): 
        for a in support:
            x1 = nconv(x, a, power)
        h = self.conv(x1)
        h = F.dropout(h, self.dropout, training=self.training)
...
class mifGCN(nn.Module):
    ...
    def forward(self, x, x_1):  
        ...
        x1 = x1+self.diffusion_conv1(x1, [learn_adj], 3) 
...
class mif_Tree(nn.Module):
    def __init__(self, device, num_nodes, channels, adj, dtw, num_levels, dropout, seq_len, num_of_x,adj_mx, pre_adj=None, pre_adj_len=1):
        ...
        self.mifGCN1 = mifGCN(splitting=True, channels=channels, seq_len=seq_len, num_of_x=num_of_x,device=device,
                            pre_adj=pre_adj, num_nodes=num_nodes, dropout=dropout, pre_adj_len=pre_adj_len,
                            adj=adj,dtw=dtw,adj_mx=adj_mx)
        self.mifGCN2 = mifGCN(splitting=True, channels=channels, seq_len=seq_len, num_of_x=num_of_x,device=device,
                            pre_adj=pre_adj, num_nodes=num_nodes, dropout=dropout, pre_adj_len=pre_adj_len,
                            adj=adj,dtw=dtw,adj_mx=adj_mx)
        self.mifGCN3 = mifGCN(splitting=True, channels=channels, seq_len=seq_len, num_of_x=num_of_x,device=device,
                            pre_adj=pre_adj, num_nodes=num_nodes, dropout=dropout, pre_adj_len=pre_adj_len,
                            adj=adj,dtw=dtw,adj_mx=adj_mx)
    ...
    def forward(self, x, x1):
        x_even_update1, x_odd_update1, dadj1, x2 = self.mifGCN1(x, x1)
        x_even_update2, x_odd_update2, dadj2, x3 = self.mifGCN2(x_even_update1, x2)
        x_even_update3, x_odd_update3, dadj3, x4 = self.mifGCN3(x_odd_update1, x3)
...
class MIFGCN(nn.Module):
    def __init__(self, device, num_nodes, seq_len,channels, dropout, adj,dtw, num_of_weeks, num_of_days,num_of_hours, adj_mx,pre_adj=None):
        ...
        self.model1 = model1(device=device, num_nodes=num_nodes, seq_len=seq_len, num_of_weeks=num_of_weeks,
                                channels=channels, dropout=dropout, pre_adj=pre_adj, adj=adj,dtw=dtw,adj_mx=self.adj)
        self.model2 = model2(device=device, num_nodes=num_nodes, seq_len=seq_len, num_of_days=num_of_days,
                          channels=channels, dropout=dropout, pre_adj=pre_adj,adj=adj,dtw=dtw,adj_mx=self.adj)
        self.model3 = model3(device=device, num_nodes=num_nodes, seq_len=seq_len, num_of_hours=num_of_hours,
                                 channels=channels, dropout=dropout, pre_adj=pre_adj,adj=adj,dtw=dtw,adj_mx=self.adj)

    def forward(self, input):
        x1 = self.model1(input['input1'])
        x2 = self.model2(input['input2'])
        x3 = self.model3(input['input3'])
